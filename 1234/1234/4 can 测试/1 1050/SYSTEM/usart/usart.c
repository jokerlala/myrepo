	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((USART2_RX_STA&0x8000)==0)//接收未完成
			...
			if(USART2_RX_STA&0x4000)//接收到了0x0d
				...
				else USART2_RX_FLAG=1;//USART2_RX_STA|=0x8000;	//接收完成了 
				}
			else //还没收到0X0D
				{	
...
void USART3_IRQHandler(void)                	//串口3中断服务程序
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		USART_SendData(USART3, Res);//向串口1发送数据
		...
		if((USART3_RX_STA&0x8000)==0)//接收未完成
			...
			if(USART3_RX_STA&0x4000)//接收到了0x0d
				...
				else USART3_RX_FLAG=1;//USART3_RX_STA|=0x8000;	//接收完成了 
				}
			else //还没收到0X0D
				{	
	...
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((UART4_RX_STA&0x8000)==0)//接收未完成
			...
			if(UART4_RX_STA&0x4000)//接收到了0x0d
				...
				else UART4_RX_FLAG=1;//UART4_RX_STA|=0x8000;	//接收完成了 
				}
			else //还没收到0X0D
				{	
	...
	if(USART_GetITStatus(UART5, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((UART5_RX_STA&0x8000)==0)//接收未完成
			...
			if(UART5_RX_STA&0x4000)//接收到了0x0d
				...
				else UART5_RX_FLAG=1;//UART5_RX_STA|=0x8000;	//接收完成了 
				}
			else //还没收到0X0D
				{	
